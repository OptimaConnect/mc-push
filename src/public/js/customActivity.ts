...
type stepObject = { label: string; key: string; active?: boolean; }
...
var today = new Date();
var currentTime = today.toUTCString();
...
function updateApiStatus(endpointSelector: string, endpointStatus: boolean) {
...
function prePopulateFields(argumentsSummaryPayload: any[]) {
...
function triggerSteps(argPromotionType: string) {
...
function validateStep(stepToValidate: number) {
    ...
    if ( $("#step" + stepToValidate).find('.slds-has-error').length > 0 ) {
        ...
        if ( !validateTheDateFormat($("#automation_run_date").val()?.toString()) ) {
...
function isCharInteger(string: string) {
...
function validateTheDateFormat(dateStringFromForm?: string) {
    if (dateStringFromForm == undefined){
        return false;
    }
...
function lookupControlGroups() {
    ...
    $.ajax({
        ...
        success: function(result: { items: string | any[]; }){
...
function toggleStepError(errorStep: number, errorStatus: string) {
...
function onGotoStep (step: stepObject) {
...
function showStep(step?: stepObject, stepIndex?: number) {
...
function saveToDataExtension(payloadToSave: { step: number; key: any; value: any; type: string; }[]) {
    ...
    try {
        $.ajax({ 
            ...
            , error: function(jqXHR: any, textStatus: any, err: any){
                if ( debug ) {
                    console.log(err);
                }
            }
...
function updateExistingRow(payloadToSave: { step: number; key: any; value: any; type: string; }[]) {
...
function createAutomationSeed(payloadToSave: { step: number; key: any; value: any; type: string; }[]) {
...
function createAutomation(payloadToSave: { step: number; key: any; value: any; type: string; }[]) {
...
function buildActivityPayload() {
    ...
    var step1FormInputs = $("#step0").find<HTMLInputElement>(":input");
    var step2FormInputs = $("#step1").find<HTMLInputElement>(":input");
    var step3FormInputs = $("#step2").find<HTMLInputElement>(":input");
...
function updateSummaryPage(summaryPayload: any[]) {
...
function cleanUpKeyText(keyString: string) {
    return keyString.split("_").join(" ");
}

function cleanUpValueText(valueString: string) {
